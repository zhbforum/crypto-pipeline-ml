...
class CollectorService:
    ...
    async def one_cycle_ticker(
        self, pairs: List[str], sink: CsvSink
    ) -> Tuple[int, int, int]:
        ok = 0
        ...
        try:
            ...
            for sym in pairs:
                ...
                if price is None:
                    continue
                rows.append(
                    {"ts": now_ms, "iso_ts": iso, "symbol": sym, "price": price}
                )
                ok += 1
        except Exception:
            pass
    ...
    async def one_cycle_kline(
        self, pairs: List[str], interval: str, sink: CsvSink, concurrency: int
    ) -> Tuple[int, int, int]:
